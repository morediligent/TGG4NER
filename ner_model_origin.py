...
class Model(nn.Module):
    def __init__(self, config):
        ...
        # self.bert = AutoModel.from_pretrained(config.cache, output_hidden_states=True)
        # self.bert = AutoModel.from_pretrained(config.bert_name, cache_dir="/home/wuwj/pycode/cache/"+config.cache_path, output_hidden_states=True)
        self.bert = BertModel.from_pretrained("/home/wuwj/pycode/cache/"+config.cache_path, output_hidden_states=True)
    ...
    def forward(self, bert_inputs, grid_mask2d, dist_inputs, pieces2word, sent_length, a2c_mask):
        '''
        ...
        # the mask
        conv_inputs = torch.masked_fill(conv_inputs, a2c_mask.eq(1).unsqueeze(-1), 0.0)
