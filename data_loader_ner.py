...
def collate_fn(data):
    bert_inputs, grid_labels, grid_mask2d, pieces2word, dist_inputs, sent_length, entity_text, a2c_mask_list = map(list, zip(*data))
    ...
    a2c_mask = torch.zeros((batch_size, max_tok, max_tok), dtype=torch.bool)
    a2c_mask_mat = torch.zeros((batch_size, max_tok, max_tok), dtype=torch.bool)
    a2c_mask_list = fill(a2c_mask_list, a2c_mask_mat)

    return bert_inputs, grid_labels, grid_mask2d, pieces2word, dist_inputs, sent_length, a2c_mask_list, entity_text


class RelationDataset(Dataset):
    def __init__(self, bert_inputs, grid_labels, grid_mask2d, pieces2word, dist_inputs, sent_length, entity_text, a2c_mask_list):
        ...
        self.a2c_mask_list =  a2c_mask_list
...
def load_data_bert(config):
    with open('/home/wuwj/pycode/data/w2ner_data/{}/train.json'.format(config.dataset), 'r', encoding='utf-8') as f:
        train_data = json.load(f)
    with open('/home/wuwj/pycode/data/w2ner_data/{}/dev.json'.format(config.dataset), 'r', encoding='utf-8') as f:
        dev_data = json.load(f)
    with open('/home/wuwj/pycode/data/w2ner_data/{}/test.json'.format(config.dataset), 'r', encoding='utf-8') as f:
        test_data = json.load(f)

    with open('./a2c_mask/'+config.dataset+'_train_a2cmask_EM0.pkl', 'rb') as file:
        train_a2c_mask = pickle.load(file)
    with open('./a2c_mask/'+config.dataset+'_dev_a2cmask_EM0.pkl', 'rb') as file:
        dev_a2c_mask = pickle.load(file)
    with open('./a2c_mask/'+config.dataset+'_test_a2cmask_EM0.pkl', 'rb') as file:
        test_a2c_mask = pickle.load(file)

    # tokenizer = AutoTokenizer.from_pretrained(config.bert_name, cache_dir="./cache/")
    # tokenizer = AutoTokenizer.from_pretrained(config.bert_name, cache_dir="/home/wuwj/pycode/cache/"+config.cache_path)
    tokenizer = AutoTokenizer.from_pretrained("/home/wuwj/pycode/cache/"+config.cache_path)


    # tokenizer = AutoTokenizer.from_pretrained(config.cache)
    vocab = Vocabulary()
    ...
    train_a2c_mask = [torch.squeeze(tensor, dim=0) for tensor in train_a2c_mask]
    dev_a2c_mask = [torch.squeeze(tensor, dim=0) for tensor in dev_a2c_mask]
    test_a2c_mask = [torch.squeeze(tensor, dim=0) for tensor in test_a2c_mask]
    train_a2c_mask = [tensor.cpu() for tensor in train_a2c_mask]
    dev_a2c_mask = [tensor.cpu() for tensor in dev_a2c_mask]
    test_a2c_mask = [tensor.cpu() for tensor in test_a2c_mask]

    train_dataset = RelationDataset(*process_bert(train_data, tokenizer, vocab), train_a2c_mask)
    dev_dataset = RelationDataset(*process_bert(dev_data, tokenizer, vocab), dev_a2c_mask)
    test_dataset = RelationDataset(*process_bert(test_data, tokenizer, vocab), test_a2c_mask)
